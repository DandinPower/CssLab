...
for sent in sentences:
    ...
    #將文本的attention mask 也加入到attention_masks 列表
...
for sent in input_ids:
    pading = torch.tensor([tokenizer.pad_token_id] * (max_length - sent[0].size()[0]))
    newSent = torch.cat((sent[0],pading),0)
    #print(sent)
    newSent = newSent.reshape([1,-1])
    new_input_ids.append(newSent)

for mask in attention_masks:
    pading = torch.tensor([0] * (max_length - mask[0].size()[0]))
    newMask = torch.cat((mask[0],pading),0)
    #print(sent)
    newMask = newMask.reshape([1,-1])
    new_attention_masks.append(newMask)
    #sent[0]
# 将列表转换为 tensor
...
# 输出第 1 行文本的原始和编码后的信息
print('Original: ', sentences[0])
print('Token IDs:', new_input_ids[0])


from torch.utils.data import TensorDataset, random_split

# 将输入数据合并为 TensorDataset 对象
dataset = TensorDataset(new_input_ids, new_attention_masks, labels)
